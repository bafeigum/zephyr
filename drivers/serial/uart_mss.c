...
#define DEV_UART(dev)						\
	((struct uart_mss_regs_t *)(DEV_CFG(dev))->uart_addr)
#define DEV_DATA(dev)						\
	((struct uart_mss_data * const)(dev)->driver_data)

static unsigned char uart_mss_poll_out(struct device *dev,
				       unsigned char c)
{
	volatile struct uart_mss_regs_t *uart = DEV_UART(dev);
...
static int uart_mss_err_check(struct device *dev)
{
	volatile struct uart_mss_regs_t *uart = DEV_UART(dev);
...
static int uart_mss_fifo_fill(struct device *dev,
			      const u8_t *tx_data,
			      int size)
{
	volatile struct uart_mss_regs_t *uart = DEV_UART(dev);
...
static int uart_mss_fifo_read(struct device *dev,
			      u8_t *rx_data,
			      const int size)
{
	volatile struct uart_mss_regs_t *uart = DEV_UART(dev);
...
static void uart_mss_irq_tx_enable(struct device *dev)
{
	ARG_UNUSED(dev);
}

static void uart_mss_irq_tx_disable(struct device *dev)
{
	ARG_UNUSED(dev);
}

static int uart_mss_irq_tx_ready(struct device *dev)
{
	volatile struct uart_mss_regs_t *uart = DEV_UART(dev);
...
static int uart_mss_irq_tx_complete(struct device *dev)
{
	ARG_UNUSED(dev);
...
static void uart_mss_irq_rx_enable(struct device *dev)
{
	ARG_UNUSED(dev);
}

static void uart_mss_irq_rx_disable(struct device *dev)
{
	ARG_UNUSED(dev);
}

static int uart_mss_irq_rx_ready(struct device *dev)
{
	volatile struct uart_mss_regs_t *uart = DEV_UART(dev);
...
static void uart_mss_irq_err_enable(struct device *dev)
{
	ARG_UNUSED(dev);
}

static void uart_mss_irq_err_disable(struct device *dev)
{
	ARG_UNUSED(dev);
}

static int uart_mss_irq_is_pending(struct device *dev)
{
	volatile struct uart_mss_regs_t *uart = DEV_UART(dev);
...
static int uart_mss_irq_update(struct device *dev)
{
	return 1;
}

static void uart_mss_irq_handler(void *arg)
{
	struct device *dev = (struct device *)arg;
	struct uart_mss_data *data = DEV_DATA(dev);
 ...
 * arrives.
 */
void uart_mss_rx_thread(void *arg1, void *arg2, void *arg3)
{
	struct device *dev = (struct device *)arg1;
	volatile struct uart_mss_regs_t *uart = DEV_UART(dev);
	const struct uart_mss_device_config *const cfg = DEV_CFG(dev);
	...
	while (1) {
		if (uart->status & STATUS_RXFULL_MASK) {
			uart_mss_irq_handler(dev);
		}
		k_sleep(delay);
	}
}

static void uart_mss_irq_callback_set(struct device *dev,
...
static const struct uart_driver_api uart_mss_driver_api = {
	...
	.err_check        = uart_mss_err_check,
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	.fifo_fill        = uart_mss_fifo_fill,
	.fifo_read        = uart_mss_fifo_read,
	.irq_tx_enable    = uart_mss_irq_tx_enable,
	.irq_tx_disable   = uart_mss_irq_tx_disable,
	.irq_tx_ready     = uart_mss_irq_tx_ready,
	.irq_tx_complete  = uart_mss_irq_tx_complete,
	.irq_rx_enable    = uart_mss_irq_rx_enable,
	.irq_rx_disable   = uart_mss_irq_rx_disable,
	.irq_rx_ready     = uart_mss_irq_rx_ready,
	.irq_err_enable   = uart_mss_irq_err_enable,
	.irq_err_disable  = uart_mss_irq_err_disable,
	.irq_is_pending   = uart_mss_irq_is_pending,
	.irq_update       = uart_mss_irq_update,
	.irq_callback_set = uart_mss_irq_callback_set,
#endif
};

#ifdef CONFIG_UART_MSS_PORT_0

static struct uart_mss_data uart_mss_data_0;

#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static void uart_mss_irq_cfg_func_0(struct device *dev);
#endif

static const struct uart_mss_device_config uart_mss_dev_cfg_0 = {
	.uart_addr    = DT_MSS_UART_0_BASE_ADDR,
	.sys_clk_freq = DT_MSS_UART_0_CLOCK_FREQUENCY,
	.line_config  = MSS_UART_0_LINECFG,
	.baud_rate    = DT_MSS_UART_0_BAUD_RATE,
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	.cfg_func     = uart_mss_irq_cfg_func_0,
#endif
};

DEVICE_AND_API_INIT(uart_mss_0, DT_MSS_UART_0_NAME,
		    uart_mss_init, &uart_mss_data_0, &uart_mss_dev_cfg_0,
...
static void uart_mss_irq_cfg_func_0(struct device *dev)
{
	...
	k_thread_create(&rx_thread, rx_stack, 500,
			uart_mss_rx_thread, dev, NULL, NULL, K_PRIO_COOP(2),
			0, K_NO_WAIT);
}
#endif

#endif /* CONFIG_MSS_UART_0 */
